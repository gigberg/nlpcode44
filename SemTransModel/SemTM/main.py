...
import dataload
import utils
from models.SentTrans import SentTransModel
...
class Trainer(object):
    def __init__(self, model):
        ...
        if config.classifier == 'SigmoidClassifier':
            self.consist_criterion = nn.BCELoss()
        ...
        self.scheduler = get_linear_schedule_with_warmup(self.optimizer, num_warmup_steps=config.warm_factor * updates_total, num_training_steps=updates_total) # 有warmup增加过程的话, 可以使用大学习率1e-3而非1e-5
    ...
    def train(self, epoch, data_loader):
        ...
        for i, data_batch in tqdm(enumerate(data_loader), total=len(data_loader), desc="Train Step Starts:"):
            ...
            # sigmoid with bce loss(1d)
            if config.classifier == 'SigmoidClassifier':
                consist_loss = self.consist_criterion(consist_out, consist_label.float())
                consist_pred = (consist_out > 0.5).squeeze().int()
            ...
            if config.flow_type == 'FlowRemove' or config.dia_enc == 'DiaRemove':
                loss = consist_loss
            else:
                loss = spk_loss + flow_loss + consist_loss
            loss.backward()
            ...
            # spk f1
            loss_array_spk.append(spk_loss.item())
            pred_result_spk.append(torch.argmax(spk_output, -1).cpu())
            label_result_spk.append(spk_label.cpu())
        ...
        # spk f1
        label_result_spk = torch.cat(label_result_spk)
        pred_result_spk = torch.cat(pred_result_spk)
        p, r, f1, _ = precision_recall_fscore_support(label_result_spk.numpy(), #numpy for sklearn
                                                      pred_result_spk.numpy(),
                                                      average='macro')
        loss_spk = np.mean(loss_array_spk)
        table.add_row(["Label_spk()", "{:.4f}".format(loss_spk)] +
                      ["{:3.4f}".format(x) for x in [f1, p, r]])
    ...
    def eval(self, epoch, data_loader, is_test=False):
        ...
        label_result_spk = torch.cat(label_result_spk)
        pred_result_spk = torch.cat(pred_result_spk)
        p, r, f1_spk, _ = precision_recall_fscore_support(label_result_spk.numpy(), #numpy for sklearn
                                                      pred_result_spk.numpy(),
                                                      average='macro')
        table.add_row(["Label_spk"] + ["{:3.4f}".format(x) for x in [f1_spk, p, r]])
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dia_enc', type=str, default="MultiUniAtten", help="MultiUniAtten, MultiSpanRNN, MultidilaCNN, MultiUniGCN, DiaRemove")
    parser.add_argument('--sent_enc', type=str, default="SentIdentity", help='SentUniAtten, SentIdentity')
    parser.add_argument('--flow_type', type=str, default="FlowCellQA", help="FlowCellQA, FlowCellQQ, FlowCellQE, FlowRemove")
    parser.add_argument('--classifier', type=str, default="SigmoidClassifier", help="SigmoidClassifier, SoftmaxClassifier, MLPClassifier, BiaffineClassifier")
    ...
    parser.add_argument('--flow_size', type=int, default=552//2) # 2
    ...
    # -----------------------------
    parser.add_argument('--consist_epochs', type=int, default=8)
    parser.add_argument('--consist_learning_rate', type=float, default=1e-4)
    ...
    if config.evaluate:
        ...
        # trainer.save(f"./checkpoint/model_{config.dataset}.pkl")
        # trainer.load(f"./checkpoint/model_{config.dataset}.pkl")
