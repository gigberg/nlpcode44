...
class GraphConvolution(nn.Module):
    def __init__(self, in_features, out_features, edge_types, dropout=0.5, bias=True, use_bn=False, pooling='max'):
        """
    ...
    def forward(self, input, adj):
        ...
        **output**: FloatTensor, output feature tensor with the same size of input, (batch_size, seq_len, hidden_size)
        ...
        for i in range(self.edge_types):
            gate_status = torch.sigmoid(self.Gates[i](input))
            assert len(adj_[i].shape) == 3
            adj_hat_i = adj_[i] * gate_status
            ts.append(torch.bmm(adj_hat_i, self.GraphConv[i](input)))
        ts = torch.stack(ts).sum(dim=0, keepdim=False).to(input.device)
