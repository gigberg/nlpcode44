...
class SentTransModel(nn.Module):
    ...
    def __init__(self, config):
        ...
        if config.dia_enc == 'DiaRemove':
            self.spk_output_layer = SpeakerMatchLayer(config.hidden_size)
        else:
    ...
    def shuffled_spk(self, sent_emb, cls_index):
        ...
        spk_mask = _label_matrix.ne(0) # 2024-5-7 label_matrix最好换成三角,防止重复评价每对关系两次,其实也没啥影响?
        spk_mask = spk_mask.tril()
...
class DialogueEncoder(nn.Module):
    ...
    def forward(self, word_reps, sent_entity, cls_index, edge_matrix, pad_mask):
        ...
        # DiaRemove
        if self.dia_enc == 'DiaRemove':
            dialog_hidden = word_reps
        else:
...
class FlowCellBase(nn.Module):
    def __init__(self, config):
        super().__init__()
        if config.dia_enc == 'DiaRemove':
            self.flow_trans = nn.Linear(config.hidden_size, config.hidden_size) # d_model, d_ff, dropout=0.1
        else:
...
class FlowCellQA(FlowCellBase): #原论文中W=ones
    ...
    Output: sent_a_hat_n, sent_a_n, flow_loss # 2024-5-7最好监视使用不同的权重?
    """
    def __init__(self, config):
    ...
    def forward(self, sent_emb, entity_emb): # both [bat, sent/entity, hid]
        ...
        _a_in, _a_target = [], []
        for dia in sent_emb:
            dia = dia[dia.sum(-1).ne(0)] #去除pad语句 # q1,a1,q2,a2,q3,a3
            _q_in.extend(dia[0:-2:2]) # q1,q2
            _q_target.extend(dia[1:-2:2]) # stat,stop,step, # a1,a2
            _a_in.extend(dia[1:-2:2]) # a1,a2
            _a_target.extend(dia[2::2]) # q2,q3
            qa_last.append(dia[-2:]) # q3,a3
        #a->q
...
class ConsistClassifier(nn.Module):
    ...
    def __init__(self, config): # word2vec.input_dim = 100
        ...
        self.module_dict = {
            ...
            'SigmoidClassifier': SigmoidClassifier()
            }
...
class SoftmaxClassifier(nn.Module):
    def __init__(self):
        super().__init__()
        self.fc1 = nn.Linear(552, 276)  # Input size: 3 (vector dimension), Output size: 2
        self.fc2 = nn.Linear(276, 2)  # Input size: 2, Output size: 1
...
class SigmoidClassifier(nn.Module):
    ...
    def forward(self, x1, x2):
        x = torch.norm(x1-x2, p=2, dim=-1, keepdim=True) # [0, +正无穷)
        x = F.sigmoid(self.linear(x)).squeeze()
        # x = F.softmax(self.fc2(x)) #cross entropy包含softmax, softmax导数梯度较小,两个一起可能会梯度消失
        return x #注意返回1d shape([8])
